...
img.src = 'img/ss_meteor.png'; // contains an image of size 256x16
// with 16 frames of size 16x16

function draw_anim(context, x, y, iobj) { // context is the canvas 2d context.
    if (iobj.source != null)
        context.drawImage(iobj.source, (iobj.current % iobj.x_total_frames) * iobj.width, Math.round(iobj.current / iobj.x_total_frames - 0.5) * iobj.height,
            iobj.width, iobj.height,
            x, y, iobj.width, iobj.height);
    iobj.current = (iobj.current + 1) % (iobj.x_total_frames * iobj.y_total_frames);
